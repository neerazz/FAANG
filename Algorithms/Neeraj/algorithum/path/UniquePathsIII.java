...
public class UniquePathsIII {
    ...
    public static int uniquePathsIII(int[][] grid) {
        ...
        dfs(start, grid, rows, cols, noObstacle);
        return occ;
    }

    private static void dfs(int[] cur, int[][] grid, int rows, int cols, int noObstacle) {
        ...
        for (int[] dir : dirs) {
            ...
            if (nr < rows && nr >= 0 && nc >= 0 && nc < cols && grid[nr][nc] >= 0) {
                dfs(new int[]{nr, nc}, grid, rows, cols, noObstacle - 1);
            }
        }
