...
public class ConstructBinaryTreeFromPreorderAndInorderTraversal {
    ...
    public static void main(String[] args) {
        System.out.println("Answer is \t: " + buildTree(new int[]{3, 9, 20, 15, 7}, new int[]{9, 3, 15, 20, 7}) + "\nshould be \t: " + TraverseATree.createTreeNode(new ArrayList<>(Arrays.asList(3, 9, 20, null, null, 15, 7))));
    }

    public static TraverseATree.TreeNode buildTree(int[] preorder, int[] inorder) {
...
//        Call the recursive method starting from zero to end of array.
    ...
    private static TraverseATree.TreeNode buildTreeHelper(int[] preorder, int[] inorder, int left, int right, HashMap<Integer, Integer> hashMap) {
        if (left > right) return null;
//        Get the last value of post and create a root.
        int rootValue = preorder[rootIndex++];
        TraverseATree.TreeNode root = new TreeNode(rootValue);
