...
public class IsCircuitWirable {
    ...
    public static boolean isAnyPlacementFeasible(List<GraphVertex> graph) {
        for (GraphVertex con : graph) {
            if (con.d == -1 && !canDivide(con, 0)) {
                return false;
            }
        }
        return true;
    ...
    private static boolean canDivide(GraphVertex con, int val) {
        if (con.d != -1) return con.d == val;
        con.d = val;
        for (GraphVertex dep : con.edges) {
//            Loop through all the edges and check that the value should be able to be placed on the other side.
            if (!canDivide(dep, val ^ 1)) {
                return false;
            }
        }
        return true;
